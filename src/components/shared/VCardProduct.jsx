...
const VCardProduct = ({
  ...
  return (
    <div className='container mx-auto px-4 gap-2 my-6 relative'>
      <h2
        data-aos='fade-up'
        className='text-2xl mb-3 font-bold py-2 text-center'>
        {heading}
      </h2>
      ...
      {loading ? (
        ...
        <div
          className='flex items-center overflow-x-scroll h-[400px] scrollbar-hide overflow-hidden transition-all scrollbar-none px-2'
          ref={scrollElement}>
          <button
          ...
          {data?.map((el, index) => (
            <Link
              ...
              to={`#`}
              key={el._id}
              className='w-full group flex relative bg-white flex-col justify-between items-center min-w-[260px] m-3 h-full md:min-w-[300px] max-w-[290px] md:max-w-[320px] p-2  shadow-md rounded'>
              <div className='flex flex-col text-center'>
                ...
                <div className='absolute rounded-md duration-300 ms-6 right-0 top-8 opacity-80 group-hover:opacity-100 bg-red-500 px-2'>
                  <span className='relative mt-5 z-50 text-xs md:font-bold self-end text-white shadow-lg'>
                    {Math.floor(
                ...
                <div className='h-52 flex items-center justify-center min-w-[300px] md:min-w-[145px]'>
                  <img
                    src={el.productImage}
                    alt={el?.productName}
                ...
                <div className='p-4 grid'>
                  <h2 className='font-medium md:font-lg'>
                    {textTruncate(el?.productName, 25)}
                  </h2>
                  <p className='text-stone-500'>{el?.category}</p>
                </div>
              </div>
              <div className='flex justify-between items-center mb-2 w-full'>
                <div className='flex ms-5 flex-col gap-2'>
